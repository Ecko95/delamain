...
type ExecutorImpl = (request: { onCall: (method: string, args: unknown[]) => Promise<unknown> }) => ScriptExecution;
...
describe("runWorkflowRun", () => {
  ...
  it("halts on timeoutMs, kills the child and any spawned leaf peers", async () => {
    let killedChild = 0;
